...
async function main() {
  ...
  for (const report of reports) {
    const opportunities = z.array(OpportunitySchema).parse(report.opportunities)
    console.log(`  → ${report.scan_date}: ${opportunities.length} opportunities`)

    const updated = await updateReportCompetition(report.scan_date, opportunities)
    ...
    const res = await fetch(`${siteUrl}/api/update-competition`, {
      ...
      body: JSON.stringify({ scan_date: report.scan_date, opportunities: updated }),
    })

    if (!res.ok) {
      const body = await res.text()
      console.error(`  ✗ Failed to update ${report.scan_date} (${res.status}): ${body}`)
    } else {
